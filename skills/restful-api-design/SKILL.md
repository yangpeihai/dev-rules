...
description: "RESTful API 设计规范指南。Cover REST API design best practices including URL structure, HTTP methods, request/response formats, status codes, pagination, authentication, error handling, rate limiting, idempotency, and API documentation. Invoke when: designing new REST APIs, reviewing API endpoints, defining request/response schemas, implementing authentication/authorization, handling errors, writing API documentation, or discussing API architecture decisions."
---
...
本规范定义了企业级 RESTful API 的设计标准，涵盖 URL 设计、HTTP 方法、请求响应格式、状态码、分页、认证授权、错误处理和文档编写等内容。
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| code | Integer | 是 | 业务状态码，200 表示成功 |
| message | String | 是 | 状态描述信息 |
| data | Object/Array | 否 | 响应数据，可为 null |
| timestamp | String | 是 | ISO 8601 格式时间戳 |
| requestId | String | 推荐 | 请求唯一标识 |
| traceId | String | 推荐 | 分布式追踪 ID |
...
| 状态码 | 场景 | 说明 |
|--------|------|------|
| 200 | 通用成功 | GET、PUT、PATCH 成功 |
| 201 | 创建成功 | POST 创建资源成功 |
| 204 | 无内容 | DELETE 成功，或无返回数据 |
...
```http
GET /users?sort=createdAt           # 按 createdAt 升序
GET /users?sort=-createdAt           # 按 createdAt 降序
...
JWT Token 应包含以下声明：iss（签发者）、sub（用户唯一标识）、aud（接收方）、exp（过期时间）、iat（签发时间）、jti（Token 唯一标识）、scope（权限范围）。
...
| 策略 | 场景 | 配置示例 |
|------|------|----------|
| 全局限流 | 保护整个系统 | 10000 请求/分钟 |
| 用户限流 | 防止滥用 | 100 请求/分钟/用户 |
| 接口限流 | 保护特定接口 | 10 请求/秒/接口 |

## 幂等性设计

### 幂等键机制

POST 请求使用 `Idempotency-Key` 头实现幂等：
...
幂等键规则：
1. 由客户端生成唯一标识（UUID 推荐）
2. 服务端缓存幂等键及响应（建议 24 小时）
3. 相同幂等键的请求返回相同响应
