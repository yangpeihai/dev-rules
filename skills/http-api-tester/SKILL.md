...
帮助 AI 或开发者在已确认的测试范围内完成 HTTP API 的测试设计、请求执行、结果记录与失败归因，并输出可复测、可追踪、可交接的结构化结果。
...
在开始设计或执行测试前，必须先明确本轮测试范围，不得默认扩大。

### 范围优先级

1. 用户显式指定的测试范围
2. 当前任务直接相关的接口变更范围
3. 为验证结论所必需的最小关联接口或最小回归范围
4. 仅在用户明确要求时，才扩大为接口组回归或更大范围测试

### 常见范围类型

| 范围类型 | 说明 | 适用场景 |
|------|------|------|
| 单接口验证 | 仅测试单个接口的指定测试点 | 联调、提测前确认、问题复现 |
| 本次变更回归 | 围绕当前改动接口和最小必要回归点执行测试 | 接口改动验证、提交前自测 |
| 定向接口组测试 | 检查某个模块或一组相关接口 | 订单流、用户流、支付流专项验证 |
| 全量接口回归 | 检查整个服务或整套 API | 发布回归、专项验收、基线校验 |

### 默认策略

- 用户未明确说明时，默认优先按“单接口验证”或“本次变更回归”处理
- 如果当前范围不足以支持结论，可扩展到最小必要上下文
- 扩展范围时，应明确说明为什么需要扩展、扩展到了哪些接口或测试点
- 未经用户明确授权，不默认做全量接口回归
