...
**何时优先查阅**：当你设计模块边界、类结构、依赖关系、异常模型、配置对象、接口抽象或项目结构时。

**阅读顺序建议**：
1. 先看核心原则，判断当前任务是“模块设计”“依赖组织”还是“异常/配置设计”。
2. 只读取与当前问题直接相关的章节，不整篇通读。
3. 如果项目已有稳定结构，优先在现有结构内做增量设计，不强行重做架构。

**默认决策倾向**：
- 要存储数据：优先考虑 `dataclass`、`TypedDict` 或更合适的数据模型
- 要定义接口：优先考虑 `Protocol` 或 ABC
- 要复用逻辑：优先组合而非继承
- 要便于测试：优先依赖注入和纯函数边界
- 要管理配置：优先显式配置对象，而不是散落的全局变量
