...
**何时查阅本规范**：当你处理大数据集、优化查询性能、设计并发方案、处理 I/O 操作、选择数据结构、编写单元测试时。

**核心规则（必须遵守）**：
1. 大数据集使用生成器（禁止一次性加载）
2. 字符串拼接使用 join（禁止循环中使用+）
3. 成员检查使用 set（禁止用 list 查找）
4. 外部调用必须设置超时
5. 数据库操作使用连接池
6. 测试覆盖率不低于 80%

**性能选型决策**：
- I/O 密集型（HTTP/DB） → asyncio 或 ThreadPoolExecutor
- CPU 密集型（计算） → ProcessPoolExecutor
- 大文件读取 → 生成器逐行读取
- 高频查找 → set/dict 替代 list
- 队列操作 → deque 替代 list
