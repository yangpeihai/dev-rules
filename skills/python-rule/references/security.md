...
**何时优先查阅**：当你处理外部输入、数据库查询、密码、文件路径、密钥、日志、序列化/反序列化、HTTP 调用或并发共享状态时。

**阅读顺序建议**：
1. 先判断风险类别：输入验证、SQL、文件、密码、密钥、序列化还是日志。
2. 只读取相关章节，不为一个局部问题整篇通读。
3. 输出结论时优先指出真实漏洞和攻击面，不把一般风格问题当安全问题。

**默认决策倾向**：
- 所有外部输入默认做校验和边界约束
- 数据库访问默认使用参数化查询或 ORM 安全接口
- 密码默认使用成熟 KDF/哈希方案，如 `bcrypt`、`argon2`
- 密钥默认从环境变量或安全配置注入，不硬编码
- 日志默认脱敏，禁止输出口令、Token、密钥和隐私数据
