...
- `.api` 文件优先表达稳定接口契约，而不是承载过多实现细节。
- 命名、分组、前缀和中间件约定应在团队内保持一致。
- 接口定义优先考虑可读性和演进成本。
...
- 将公共请求/响应片段抽到基础 `.api` 文件中复用，但避免过度嵌套导致阅读困难。
- 对外接口优先保持路径、命名、分页和错误语义一致。
- 变更 `.api` 时同步检查生成代码、文档和调用方兼容性。

## 禁止事项

- ❌ 在 `.api` 中混入明显实现细节或临时约定
- ❌ 同一服务中出现多套不一致的路径和命名风格
- ❌ 修改接口契约后不验证生成代码和兼容性
