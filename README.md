...
- 用 `skills/` 提供按场景加载的技能说明，覆盖开发、审查、安全、Git、API 设计等常见任务。
...
└── skills/
    ...
    ├── restful-api-design/
    ├── secure-development/
    ├── spec-ai-requirement-analyst/
    ├── spec-raw-requirement-ai-analyst/
    └── theme-factory/
```
...
`spec-ai-requirement-analyst` 与 `spec-raw-requirement-ai-analyst` 目录当前也已存在，可作为需求分析类能力扩展的占位目录继续补充。
